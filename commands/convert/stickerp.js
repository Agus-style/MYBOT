...
module.exports = {
    ...
    start: async(caf, m, { command, prefix, quoted, mime }) => {
        if (!quoted) return  m.reply(`Reply to Supported media With Caption ${prefix + command}`)
        if (/image|video|sticker/.test(mime)) {
            let download = await caf.downloadAndSaveMediaMessage(quoted)
            file_name = getRandom('webp')
            ffmpeg(`./${download}`).input(download).on('end', function () {
                caf.sendFile(m.from, fs.readFileSync(file_name), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] }).then(() => {
