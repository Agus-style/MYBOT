const { extensionForMediaMessage, extractMessageContent, jidNormalizedUser, getContentType, normalizeMessageContent, proto, delay, downloadContentFromMessage, getBinaryNodeChild, generateWAMessageFromContent, jidDecode } = require('@whiskeysockets/baileys')
...
class WAConnection {
    ...
    async linkUp(jid, tex, url, img, ttle, bdy, qtd = []) {
        ...
        return this.relayMessage(jid, template.message, { messageId: template.key.id })
    }

    async sendPoll(jid, text, poll = { options: ["Opsi 1", "Opsi 2"], singleSelect: false }) {
        if (poll.options.length < 2) return false
        return this.sendMessage(jid, { 
            poll: { 
                name: text, 
                values: poll.options, 
                selectableCount : (poll.singleSelect ? 1 : {})
            }
        })
    }
}
